...
output_directory = '/Users/micahdemarest/Desktop/coding/sniper_bot/output'  # Adjust this path to your existing output folder
dud_files_directory = os.path.join(output_directory, 'duds')
corrected_files_directory = os.path.join(output_directory, 'corrected')

os.makedirs(dud_files_directory, exist_ok=True)
...
def create_and_run_backtest(strategy_output, trading_idea):
    if strategy_output:
        ...
        instructions = (
            ...
            "The script does not need to use backtesting.py or any actual historical data files, but you should "
            "implement aspects from the library given what you know."
...
def is_dud_file(file_path):
    """Determine if the file is a dud based on the absence of '.plot()'."""
    try:
        with open(file_path, 'r') as file:
            content = file.read()
        # If '.plot()' is not found in the file, it's likely a dud
...
def generate_correction_prompt(file_content):
    """Generate a detailed prompt for correcting the file."""
    prompt = ("Here is a Python script intended for backtesting a trading strategy. "
              "It might contain syntax errors, incomplete logic, or missing assumptions. "
              "Please correct these issues, ensuring the script is executable and logically consistent. Use what you "
              "know of the backtesting.py library to guide your code. Having real access to the library isn't important"
              "Make reasonable assumptions to fill in any gaps. The script follows:\n\n"
              f"{file_content}\n\n"
              "# Please provide the corrected script below.")
    return prompt


def correct_dud_file(file_path):
    """Attempt to correct issues in the dud file using GPT."""
    try:
        with open(file_path, 'r') as file:
            content = file.read()

        prompt = generate_correction_prompt(content)
        ...
        print(f"Error generating correction for {file_path}: {e}")
        return None


def correct_dud_files():
    for filename in os.listdir(dud_files_directory):  # Changed to dud_files_directory
        file_path = os.path.join(dud_files_directory, filename)  # Changed to dud_files_directory
        if os.path.isfile(file_path):  # We already know they are in 'duds', so just check if it's a file
            print(f"Correcting {filename}...")
            corrected_content = correct_dud_file(file_path)
            if corrected_content:
                corrected_file_path = os.path.join(corrected_files_directory, filename)
                try:
                    with open(corrected_file_path, 'w') as corrected_file:
                        corrected_file.write(corrected_content)
                    print(f"Saved corrected file to {corrected_file_path}")
                    # Optionally, move the original dud file to an archive directory
                except Exception as e:
                    print(f"Failed to save corrected file {filename}: {e}")
            else:
                print(f"Failed to correct {filename}")
